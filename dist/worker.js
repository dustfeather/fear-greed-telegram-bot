// Built for Terraform deployment
...
var import_whatwg_url, Readable, BUFFER, TYPE, Blob, convert, INTERNALS, PassThrough, invalidTokenRegex, invalidHeaderCharRegex, MAP, Headers2, INTERNAL, HeadersIteratorPrototype, INTERNALS$1, STATUS_CODES, Response2, INTERNALS$2, URL2, parse_url, format_url, streamDestructionSupported, Request, URL$1, PassThrough$1, isDomainOrSubdomain, isSameProtocol, lib_default;
var init_lib = __esm({
  "node_modules/cross-fetch/node_modules/node-fetch/lib/index.mjs"() {
...
var require_object = __commonJS({
  "node_modules/javascript-stringify/dist/object.js"(exports) {
    ...
    var objectToString = (value, space, next, key) => {
      ...
      if (typeof globalThis === "object" && value === globalThis) {
...
function executionHistoryKey(chatId) {
  return `execution_history:${chatId}`;
}
function activePositionKey(chatId) {
  return `active_position:${chatId}`;
}
function watchlistKey(chatId) {
...
var CACHE_CONFIG = {
  FEAR_GREED_TTL_SECONDS: 300,
  // 5 minutes
  FEAR_GREED_TTL_MS: 3e5
};
var RATE_LIMITS = {
...
var TRADING_CONFIG = {
  ...
  BOLLINGER_PERIOD: 20,
  BOLLINGER_STDDEV: 2,
  BB_LOWER_THRESHOLD: 0.01,
  // 1% above lower band
  BB_UPPER_THRESHOLD: 0.01,
  // 1% above upper band
  TRADING_FREQUENCY_DAYS: 30,
  // Deprecated: now using calendar month restriction
  HISTORICAL_DAYS_NEEDED: 200
  // Need 200 days for SMA 200
};

// src/utils/errors.ts
var AppError = class _AppError extends Error {
  constructor(type, message, cause, statusCode) {
    super(message);
    this.type = type;
    this.cause = cause;
...
function createNetworkError(message, cause) {
  return new AppError("NETWORK" /* NETWORK */, message, cause, 503);
}
function createApiError(message, cause, statusCode) {
  return new AppError("API" /* API */, message, cause, statusCode || 500);
}
function createKVError(message, cause) {
  return new AppError("KV" /* KV */, message, cause, 500);
}
function toAppError(error) {
  ...
  if (error instanceof Error) {
    return new AppError("UNKNOWN" /* UNKNOWN */, error.message, error);
  }
  return new AppError("UNKNOWN" /* UNKNOWN */, String(error), error);
}
...
async function addChatId(kv, chatId) {
  try {
...
async function removeChatId(kv, chatId) {
  try {
    const chatIds = await getChatIds(kv);
...
function isValidFearGreedIndexResponse(data) {
  ...
  if (obj.timestamp !== void 0 && typeof obj.timestamp !== "number" && typeof obj.timestamp !== "string") {
    return false;
  }
  return true;
}
function isValidTelegramApiResponse(data) {
  if (!data || typeof data !== "object") {
    return false;
  }
  const obj = data;
  ...
  if (obj.error_code !== void 0 && typeof obj.error_code !== "number") {
    return false;
  }
...
var DEFAULT_WATCHLIST = ["SPY"];
function normalizeAndDeduplicate(tickers) {
...
async function initializeWatchlistIfMissing(kv, chatId) {
  try {
...
async function getWatchlist(kv, chatId) {
  try {
    const key = watchlistKey(chatId);
    const watchlistString = await kv.get(key);
    if (!watchlistString) {
...
async function setWatchlist(kv, chatId, tickers) {
  try {
    const key = watchlistKey(chatId);
...
async function addTickerToWatchlist(kv, chatId, ticker) {
  try {
...
async function removeTickerFromWatchlist(kv, chatId, ticker) {
  try {
    const validation = isValidTicker(ticker);
    if (!validation.isValid) {
      return {
...
async function ensureTickerInWatchlist(kv, chatId, ticker) {
  try {
    const validation = isValidTicker(ticker);
    if (!validation.isValid) {
      return;
...
async function fetchWithTimeout(url, options = {}, timeoutMs = REQUEST_CONFIG.TIMEOUT_MS) {
...
async function enhancedFetch(url, options = {}, retryConfig) {
...
async function listSubscribers(env) {
  try {
    ...
    const delayBetweenBatches = 1e3 / RATE_LIMITS.TELEGRAM_MESSAGES_PER_SECOND * batchSize;
    for (let i = 0; i < chatIds.length; i += batchSize) {
      ...
      results.forEach((result, batchIndex) => {
        ...
        if (result.status === "fulfilled" && result.value) {
          ...
          if (chatInfo.username) {
            ...
            const fullName = chatInfo.last_name ? `${chatInfo.first_name} ${chatInfo.last_name}` : chatInfo.first_name;
      ...
      if (i + batchSize < chatIds.length) {
        await sleep2(delayBetweenBatches);
      }
    }
...
var USER_AGENT = `quickchart-js/3.1.0`;
...
async function broadcastToAllSubscribers(message, env) {
  try {
    const chatIds = await getChatIds(env.FEAR_GREED_KV);
    if (chatIds.length === 0) {
      return {
    ...
    for (let i = 0; i < chatIds.length; i += batchSize) {
      ...
      if (i + batchSize < chatIds.length) {
        await sleep3(delayBetweenBatches);
      }
    }
    return {
      totalSubscribers: chatIds.length,
...
// src/utils/cache.ts
async function getCachedFearGreedIndex(kv) {
  try {
...
async function cacheFearGreedIndex(kv, data) {
  try {
    const cachedData = {
      data,
      timestamp: Date.now()
...
function calculateBollingerBands(prices, period = TRADING_CONFIG.BOLLINGER_PERIOD, stdDev = TRADING_CONFIG.BOLLINGER_STDDEV) {
  if (prices.length < period) {
...
// src/market-data.ts
async function fetchMarketData(ticker = "SPY") {
  const validation = isValidTicker(ticker);
  if (!validation.isValid) {
  ...
  for (let i = 0; i < dataLength; i++) {
    if (timestamps[i] && opens[i] != null && highs[i] != null && lows[i] != null && closes[i] != null && volumes[i] != null) {
      historicalData.push({
        date: timestamps[i],
        open: opens[i],
        high: highs[i],
...
// src/utils/executions.ts
async function recordExecution(kv, chatId, signalType, ticker, executionPrice, signalPrice, executionDate) {
  try {
...
async function getExecutionHistory(kv, chatId, ticker) {
  try {
    const key = executionHistoryKey(chatId);
    const historyString = await kv.get(key);
    if (!historyString) {
...
async function getLatestExecution(kv, chatId, ticker) {
  try {
...
function formatExecutionHistory(executions) {
...
`);
  for (const exec of executions) {
    ...
    const signalEmoji = exec.signalType === "BUY" ? "\u{1F7E2}" : "\u{1F534}";
...
// src/utils/trades.ts
async function getActivePosition(kv, chatId) {
  try {
...
async function setActivePosition(kv, chatId, ticker, entryPrice) {
  try {
    const key = activePositionKey(chatId);
    const position = { ticker, entryPrice };
    await kv.put(key, JSON.stringify(position));
  } catch (error) {
    throw createKVError("Failed to set active position", error);
  }
}
async function canTrade(kv, chatId) {
  try {
    ...
    const sameMonth = now.getUTCFullYear() === lastExecutionDate.getUTCFullYear() && now.getUTCMonth() === lastExecutionDate.getUTCMonth();
    return !sameMonth;
  } catch (error) {
...
async function clearActivePosition(kv, chatId) {
  try {
    const key = activePositionKey(chatId);
    await kv.delete(key);
  } catch (error) {
...
// src/trading-signal.ts
function evaluateConditionA(currentPrice, indicators) {
  ...
  return currentPrice <= sma20Threshold && currentPrice <= lowerBBThreshold || currentPrice <= sma50Threshold || currentPrice <= sma100Threshold || currentPrice <= sma200Threshold;
}
function evaluateConditionB(currentPrice, indicators) {
  const threshold = indicators.bollingerLower * (1 + TRADING_CONFIG.BB_LOWER_THRESHOLD);
  return currentPrice <= threshold;
}
function evaluateConditionC(fearGreedData) {
  const rating = fearGreedData.rating.toLowerCase();
  return rating === RATINGS.FEAR || rating === RATINGS.EXTREME_FEAR;
}
function calculateAllTimeHigh(historicalData) {
...
function generateReasoning(signal, conditionA, _conditionB, conditionC, indicators, hasActivePosition = false, sellTarget, currentPrice, exitContext = {}) {
  const reasons = [];
...
function createDataUnavailableSignal(fearGreedData, ticker = "SPY") {
  ...
  const reasons = ["HOLD - Insufficient data to evaluate trading conditions"];
  if (!fearGreedData) {
...
async function evaluateTradingSignal(env, fearGreedData, ticker = "SPY", chatId) {
  ...
  let entryPrice;
  let exitTrigger;
  let bollingerSellTargetValue;
  let hasPositiveProfitFlag;
  if (activePosition) {
...
function formatTradingSignalMessage(signal, fearGreedData, ticker = "SPY") {
  const { signal: signalType, currentPrice, indicators, reasoning, entryPrice, sellTarget } = signal;
  const isDataUnavailable = currentPrice === 0 && indicators.sma20 === 0 && indicators.sma50 === 0 && indicators.sma100 === 0 && indicators.sma200 === 0;
  const signalEmoji = signalType === "BUY" ? "\u{1F7E2}" : signalType === "SELL" ? "\u{1F534}" : "\u{1F7E1}";
...
`;
    ...
    message += `\u{1F4B0} Current [${ticker} Price](${tradingViewUrl}): $${currentPrice.toFixed(2)}
...
`;
    ...
    if (entryPrice) {
      message += `\u{1F4C8} Entry Price: $${entryPrice.toFixed(2)}
`;
    }
    if (sellTarget) {
      const targetLabel = signal.exitTrigger === "BOLLINGER_UPPER" ? "Sell Target (BB Upper)" : signal.exitTrigger === "ALL_TIME_HIGH" ? "Sell Target (ATH)" : "Sell Target";
      message += `\u{1F3AF} ${targetLabel}: $${sellTarget.toFixed(2)}
`;
    }
    if (signal.bollingerSellTarget && (sellTarget === void 0 || signal.bollingerSellTarget !== sellTarget)) {
      message += `\u{1F3AF} BB Upper Target: $${signal.bollingerSellTarget.toFixed(2)}
`;
    }
  }
  message += `
...
async function fetchFearGreedIndex() {
  ...
  const normalized = {
    rating: obj.rating,
    score: typeof obj.score === "string" ? parseFloat(obj.score) : obj.score,
    timestamp: obj.timestamp,
    // Include optional additional fields if present
    ...obj.previous_close !== void 0 && { previous_close: typeof obj.previous_close === "string" ? parseFloat(obj.previous_close) : obj.previous_close },
    ...obj.previous_1_week !== void 0 && { previous_1_week: typeof obj.previous_1_week === "string" ? parseFloat(obj.previous_1_week) : obj.previous_1_week },
    ...obj.previous_1_month !== void 0 && { previous_1_month: typeof obj.previous_1_month === "string" ? parseFloat(obj.previous_1_month) : obj.previous_1_month },
    ...obj.previous_1_year !== void 0 && { previous_1_year: typeof obj.previous_1_year === "string" ? parseFloat(obj.previous_1_year) : obj.previous_1_year }
  };
  return normalized;
}
async function handleScheduled(chatId = null, env, ticker) {
  try {
    ...
    if (chatId) {
      ...
      for (const tickerSymbol of tickersToProcess) {
        ...
        const shouldIncludeFearGreed = rating === RATINGS.FEAR || rating === RATINGS.EXTREME_FEAR || ticker !== void 0;
...
${tradingSignalMessage}`;
    ...
    if (chatId) {
      try {
        ...
        const tickersToProcess = ticker ? [ticker] : watchlist;
        for (const tickerSymbol of tickersToProcess) {
          const dataUnavailableSignal = createDataUnavailableSignal(void 0, tickerSymbol);
          const tradingSignalMessage = formatTradingSignalMessage(dataUnavailableSignal, void 0, tickerSymbol);
          await sendTelegramMessage(chatId, tradingSignalMessage, env);
        }
      } catch (signalError) {
...
function errorResponse(error, status = 500, headers) {
  return jsonResponse({ ok: false, error }, status, headers);
}
function unauthorizedResponse(error = "Unauthorized") {
  return errorResponse(error, 401);
}
function badRequestResponse(error = "Bad Request") {
  return errorResponse(error, 400);
}
function methodNotAllowedResponse(message = "Method not allowed") {
  ...
  if (request.method === "POST" && pathname === "/") {
    ...
    try {
      if (text === COMMANDS.START) {
        ...
        try {
          await recordExecution(env.FEAR_GREED_KV, chatId, signalType, ticker, executionPrice, void 0, executionDate);
          if (signalType === "BUY") {
          ...
          const signalEmoji = signalType === "BUY" ? "\u{1F7E2}" : "\u{1F534}";
...
${formatted}`,
        ...
        if (parts.length === 1) {
          try {
            const watchlist = await getWatchlist(env.FEAR_GREED_KV, chatId);
            const watchlistText = watchlist.length > 0 ? watchlist.map((t) => `\u2022 $${t}`).join("\n") : "\u2022 $SPY (default)";
            await sendTelegramMessage(
              chatId,
              `*Your Watchlist:*
...
async function handleDeployNotify(request, env) {
  try {
    ...
    const message = `\u{1F680} New version deployed!

Commit: \`${commitHash}\`
Message: ${commitMessage.split("\n")[0]}

\u{1F517} [View on GitHub](${commitUrl})`;
